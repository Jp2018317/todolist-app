...
export default function AddTask() {
  ...
  const [taskValue, setTaskValue] = useState<{title: string, status: "Complete" | "Incomplete"}>({title: "", status: "Incomplete"});

  async function newTask() {
    console.log("añadir task");
    console.log({TASKS_VALUES: taskValue})
    await addNewTask({title: taskValue.title, status: taskValue.status, author: "jmorales317"});
              ...
              <div className="absolute w-full right-0 mt-2 bg-indigo-50 border border-indigo-200 flex flex-col justify-between items-center rounded-md font-medium">
                {
                  listFilter.map((filter) => (
                    <button
                      key={filter}
                      onClick={() => { setFilterValue(`${filter}`); setOpenFilter(!openFilter) }}
        ...
        <>
          ...
          <div className="fixed top-1/3 left-1/2 -translate-x-1/2 z-50 w-full max-w-md bg-indigo-50 rounded-lg p-5 space-y-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              />
              {
                taskValue.title.length < 3 && (
                  <span  className="text-xs text-red-500 font-medium">Title must have more than 3 characters</span>
                )
              }
              <span className="text-sm text-gray-500 font-medium">Status</span>
              <div className="relative">
                <button onClick={() => setStatusFilter(!statusFilter)} className="w-full bg-white flex justify-between items-center gap-x-8 px-5 py-2 rounded-md font-medium">
                  <div>
                    { taskValue.status }
                  </div>
                  <FaAngleDown />
                    ...
                    <div className="absolute w-full right-0 bg-indigo-50 border border-indigo-200 flex flex-col justify-between items-center rounded-sm font-medium">
                      {
                        statusDropdown.map((status) => (
                          <button
                            key={status}
                            onClick={() => { setTaskValue({...taskValue, status: status === "Complete" ? "Complete" : "Incomplete"}); setStatusFilter(!statusFilter) }}
                            className="w-full hover:bg-indigo-100 active:bg-indigo-200 border-b border-indigo-200 text-left gap-x-2 px-4 py-1.5 font-medium"
                          >
                            {status}
                          </button>
            ...
            <div className="w-full flex gap-2">
              ...
              <button
                onClick={() => {setShowAddTask(false); setTaskValue({title: "", status: "Incomplete"})}}
